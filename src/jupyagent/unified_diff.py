...
def apply_unified_diff(source: str, hunks: list[Hunk]) -> str:
    ...
    for hunk in hunks:
        ...
        if start < cursor or start > len(original_lines):
            raise JupyagentError("error: patch hunk location is invalid")
        ...
        for line in hunk.lines:
            if line.operation == " ":
                _expect_source_line(original_lines, cursor, line.text)
...
def _expect_source_line(source_lines: list[str], index: int, expected: str) -> None:
    if index >= len(source_lines) or source_lines[index] != expected:
        raise JupyagentError("error: patch hunk did not match cell source")
